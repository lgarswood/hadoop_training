...
object PurchaseTransformer {
  ...
  */
  ...
  def main(args: Array[String]) {
    val spark = SparkSession.builder.appName("Purchase Transformer").getOrCreate()
    import spark.implicits._

    val jdbcUrl = "jdbc:mysql://127.0.0.1:3306/lucas"
    ...
    val purchases = spark.read.csv("/user/root/events/*/*/*/*")
    ...
    val countries = spark.read.option("header","true").csv("/user/root/countries/*")
      .select($"geoname_id" cast IntegerType as "geonameId", $"country_name" as "countryName")
      .as[Country]
    val countryNetworks = spark.read.option("header","true").csv("/user/root/ips/*")
